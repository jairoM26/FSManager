...
main =  do 
 ...
 --groups,users,(storagedevices, volumegroups, logicalvolumes),(filesystem,),messageToPrint)
  fsManager ([],[],([],[],[]),([],[("/","/",["dev","Home"],[],head (splitOn " " (formatTime defaultTimeLocale "%m/%d/%Y %H:%M" myTime)),last (splitOn " " (formatTime defaultTimeLocale "%m/%d/%Y %H:%M" myTime)),"root:root","/","",False),("Home","/Home",[],[],head (splitOn " " (formatTime defaultTimeLocale "%m/%d/%Y %H:%M" myTime)),last (splitOn " " (formatTime defaultTimeLocale "%m/%d/%Y %H:%M" myTime)),"root:root","/","",False),("dev","/dev",[],[],head (splitOn " " (formatTime defaultTimeLocale "%m/%d/%Y %H:%M" myTime)),last (splitOn " " (formatTime defaultTimeLocale "%m:%d:%Y %H:%M" myTime)),"root:root","/","",False)],[])," ")
...
fsManager pGeneralList = do
    ...
    putStr ( sel7 (getDirByCD((sel2 (sel4 (pGeneralList))),sel8 (head (sel2 (sel4 (pGeneralList)))))) ++ sel8 (head (sel2 (sel4 (pGeneralList)))) ++ ":~$ ")
...
input (pGroupList,pUserList,storageList,pVolumeGroupsList,pLVolume,pFileSysList,directoriesList,filesList,x:xs,y,date,time)
      ...
      | x == "rmdir" = rmdir (pGroupList,pUserList,storageList,pVolumeGroupsList,pLVolume,pFileSysList,directoriesList,filesList,xs,y)
      | x == "ls" = ls (pGroupList,pUserList,storageList,pVolumeGroupsList,pLVolume,pFileSysList,directoriesList,filesList,xs,y)
      | x == "chown" = chown (pGroupList,pUserList,storageList,pVolumeGroupsList,pLVolume,pFileSysList,directoriesList,filesList,xs,y)
      | x == "showall" = showall (pGroupList,pUserList,storageList,pVolumeGroupsList,pLVolume,pFileSysList,directoriesList,filesList,xs,y)
