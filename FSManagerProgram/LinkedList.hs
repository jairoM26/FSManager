...
{-
  ...
  --LinkedList a: lista = pList
  --a: elemento a insertar = elemToInsert
  --LinkedList a: lista a retornar
-}
...
{-
  ...
  --Take as parameter:
    --a = element to delete
    -- LinkedList a = The list that we are going to delete the element
  --Return a list with the element delete if its exists
-}
deleteData ::  Eq a => a -> LinkedList a -> LinkedList a
deleteData elemToDelete EmptyNode = EmptyNode
deleteData elemToDelete pList = if ((getHead (pList)) == elemToDelete) then (deleteData elemToDelete (getTail pList))
                                else Cons (getHead pList) (deleteData elemToDelete (getTail pList))
